...
public class SqlUtil {
    ...
    public static <T> T mapToBean(Map<String, Object> map, Class<T> clazz) {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            List<String> snakeCaseFields = Lists.newArrayList(key.split("_")).stream().map(StringUtils::capitalize).collect(Collectors.toList());
            String methodName = "set" + StringUtils.join(snakeCaseFields, "");
            Method set = clazz.getMethod(methodName, entry.getValue().getClass() == Timestamp.class ? Date.class : entry.getValue().getClass() );
