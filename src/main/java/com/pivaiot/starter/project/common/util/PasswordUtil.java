...
public class PasswordUtil {
    ...
    public static String pbkdf2(@NonNull String password, @NonNull String salt, int iterations, int bytes) {
        if (password == null || salt == null) {
            throw new IllegalArgumentException("Password or salt cannot be null");
        }

        try {
            return HexUtil.encode(pbkdf2(password.toCharArray(), salt.getBytes(), iterations, bytes));
        } catch (Exception e) {
            LOGGER.warn("password length:{},salt:{},iterations:{},bytes:{}", password.length(), salt, iterations, bytes, e);
